...
class Loader: 
    ...
    def load(self, df: pd.DataFrame, quality_report : dict):
        try:
            ...
            for (lang, year), subset in df.groupby(["language", "year"]):
                buffer = BytesIO()
                subset.to_json(buffer, orient="records", date_format="iso")
            ...
            buffer = BytesIO()
            buffer.write(json.dumps(quality_report).encode("utf-8"))
            key = f"quality_report.json"
            s3.put_object(Bucket=self.config["bucket"], Key=key, Body=buffer.getvalue())
